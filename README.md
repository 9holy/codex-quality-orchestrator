...
| Luna Max | 优先承担目标清楚、边界明确、可以单独验证的实现、测试、扫描和批量工作 |
| Terra XHigh/Max/Ultra | 处理需要判断、排查根因、跨文件推理、疑难调试或独立复核的工作 |

简单问题由当前主控直接处理。复杂任务可以拆成多个互不冲突的工作单元时，默认同时安排 2 个、最多 3 个 Worker。架构、安全、公共接口、生产数据、不可逆操作以及最终裁决仍由 Sol 负责。

路由判断由 Sol 根据完整上下文完成。插件的 Hook 负责检查实际调用是否符合规则，限制并发和重试，记录 Worker 状态，并阻止错误模型、错误档位或越界调用。它不会用死板的关键词替代模型理解任务。

Terra XHigh 用于常规判断型工作，Max 用于疑难复杂任务，Ultra 只用于边界已经明确、但需要接近 Sol XHigh 推理强度的执行或关键只读复核。即使使用 Terra Ultra，规划、整合和最终裁决仍由 Sol 负责。
...
安装脚本只安装 `luna_worker` 和 `terra_worker` 两个具名代理。已有兼容配置会保留；发现冲突时会停止，不会静默覆盖。只有你明确决定替换冲突配置时才使用 `-Force`，脚本会先备份原文件。
...
- 插件不能改变已经启动的根任务模型或推理档位。日常团队任务建议从 Sol High 开始，复杂规划可用 XHigh，高风险裁决再用 Max。
