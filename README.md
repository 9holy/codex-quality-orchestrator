...
| Terra Max/Ultra | 需要独立判断或并行的复杂工作用 Max，最深独立推理用 Ultra；XHigh 保留兼容但不自动选择 |
...
Luna 不适用后，当前 Sol 能可靠完成且不需要独立性或并行时就直接处理。Terra Max 用于普通独立复核或必须并行的复杂单元，Terra Ultra 用于最深独立推理。即使使用 Terra，规划、整合和最终裁决仍由 Sol 负责。
