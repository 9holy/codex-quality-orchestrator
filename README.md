...
| Terra XHigh/Max/Ultra | 处理需要判断、排查根因、跨文件推理、疑难调试或独立复核的工作 |

简单问题由当前主控直接处理。通常只使用 1 个 Worker；存在 2–3 个互不冲突且确有并行收益的工作单元时才并发，最多 3 个。架构、安全、公共接口、生产数据、不可逆操作以及最终裁决仍由 Sol 负责。

路由判断由 Sol 根据完整上下文完成。插件的 Hook 负责检查实际调用是否符合规则，限制并发和重试，记录 Worker 状态，并阻止错误模型、错误档位或越界调用。它不会用死板的关键词替代模型理解任务。

Luna Max 能可靠完成且可独立验收时必须选择 Luna，不会因为雷达分数而上调。只有 Luna 不适用时，插件才向 Sol 提供 Codex Radar 的精简 IQ、平均成本、耗时和样本数，用于在已经具备任务能力的 Terra/Sol 候选中选择预计总成本最低者。数据每 6 小时最多刷新一次，超过 24 小时或样本不足时自动忽略，不影响静态路由。

Terra XHigh 用于常规判断，Max 用于高难度工作，Ultra 用于需要最深推理的工作，由 Sol 按正常任务需求选择。即使使用 Terra Ultra，规划、整合和最终裁决仍由 Sol 负责。

## 自动分配示例

- “把已经确定的三个页面按设计稿实现并分别测试”：Sol 可以拆分后交给 Luna Max 并行执行，最后统一验收。
- “查出偶发数据错乱的根因并修复”：先交给 Terra 分析和调试，必要时由 Sol 接管高风险部分。
- “修改生产数据库迁移方案”：由 Sol 负责，不会为了节省算力强行下派。
- “解释这一行代码”：属于短任务，当前主控直接回答，不创建 Worker。

如果 Worker 返回 `Selected model is at capacity. Please try a different model.`，插件只让原 Worker 在原上下文中继续尝试一次，不会把整个任务重新开始。再次失败后才按照 Luna → Terra → 当前 Sol 的顺序升级。
...
- 插件不能改变已经启动的根任务模型或推理档位。新任务建议从 Sol Medium 开始；需要多步骤逻辑和边界检查时可用 High，复杂规划优先 XHigh，最难或高风险裁决再用 Max，Ultra 只用于可有效并行的系统性多波次任务。
