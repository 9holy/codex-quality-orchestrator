...
路由判断由 Sol 根据完整上下文完成。只读不自动等于可下派，需求模糊或根因尚未确定的诊断仍由 Sol 完成。插件的 Hook 负责检查实际调用是否符合规则，限制并发和重试，记录 Worker 状态，并阻止错误模型、错误档位或越界调用。它不会用死板的关键词替代模型理解任务。

桌面端创建代理时会展示工作包开头；第二行固定显示 `route: <完整模型> / <推理等级>`，例如 `route: gpt-5.6-luna / max`。模型和档位仍由代理配置与 Hook 分别校验，随机代理昵称不作为路由证据。

Luna Max 在 Sol 判断能胜任时直接选择，不读取 Codex Radar。只有 Luna 不适用且同时存在多个能胜任候选时，Sol 才按需读取一次雷达摘要；插件每天最多刷新一次，完整数据只留在本地缓存。单一候选、缓存过期或样本不足时由 Sol 直接判断，后续仅在新增单元、边界变化、失败或模型不可用时重判。
