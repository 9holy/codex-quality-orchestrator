...
按完整工作单元最高要求选择能可靠胜任且总算力成本最低的层级，质量和风险优先。目标、边界、接口、验收已冻结且可独立验证的执行必须优先交 `luna_worker / gpt-5.6-luna / max`，包括实现、多文件修改、已定位问题的修复、测试、扫描和批量工作；需消歧、根因诊断、跨上下文推断、疑难调试或独立复核交 `terra_worker / gpt-5.6-terra / xhigh|max|ultra`：`xhigh` 常规，`max` 高难度，`ultra` 最深推理，按需要选择。架构、安全、公共接口、生产数据、不可逆迁移、公共数据契约和最终裁决留给 Sol。

Luna 能可靠胜任且可独立验收的单元必须下派；通常使用 1 个 Worker，存在 2–3 个互不冲突且并行收益大于额外算力和整合成本的单元时按需并行，最多 3 个。共享文件单写者，Worker 不得下派；无法安全拆分或 Worker 不能可靠胜任时当前 Sol 接管，不创建 Sol 子代理。
