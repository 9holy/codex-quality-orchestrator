...
按完整工作单元最高要求选择能可靠胜任且总算力成本最低的层级，质量、能力和风险优先。目标、边界、接口和验收已冻结，允许在边界内做局部实现选择且可独立验证的清晰执行，优先交 `luna_worker / gpt-5.6-luna / max`，包括中大型实现、多文件修改、常规调试、测试、扫描和批量工作；需消歧、根因诊断、跨上下文推断、疑难调试或关键只读复核交 `terra_worker / gpt-5.6-terra / xhigh|max|ultra`：xhigh 常规判断，max 疑难复杂，ultra 仅用于需要接近 Sol xhigh 推理强度但不含主控职责的冻结执行或复核；架构、安全、公共接口、生产数据、不可逆迁移、公共数据契约和最终裁决留给 Sol。
