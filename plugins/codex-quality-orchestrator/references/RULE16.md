...
通常 1 个 Worker；仅 2–3 个互不冲突且并行收益更大时并行，最多 3 个。共享文件单写者，Worker 不得下派；Sol 不创建 Sol 子代理。
...
Worker 的 `task_name` 使用 `<模型档位>__<单元>__w<波次>__s<槽位>of<波次大小>__a<尝试>`，前缀限 `luna_max|terra_ultra`。`message` 用 `[CQO_WORK_PACKET_V1]` 写明目标、范围、写路径、验收、验证、权限、备份和 `selected_agent|selected_effort|fallback`。路由键和单元须唯一，每单元最多 2 次；Sol 必须自行验收语义。
...
调用 `luna_worker` 传 `agent_type,fork_turns`；调用 `terra_worker` 另传 `reasoning_effort=ultra`；均不传 `model`。`fork_turns` 默认 `"none"`，仅需少量历史时传正整数字符串。
