...
- 任务开始时只在能胜任候选间使用一次新鲜 `[CQO_RADAR]` 数据确定执行者。Luna Max 能胜任时固定优先；否则 IQ 差≥3 选高 IQ，差<3 视为同级；同级先保留热模型/原代理，再选预计总成本更低者。没有新鲜数据就由 Sol 判断。确定后不重复选模；仅新增单元、边界变化、执行失败或模型不可用时重判。
