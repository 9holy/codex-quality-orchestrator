...
3. Apply Rule 16 to every unit. Decide Luna capability first and never trial-dispatch uncertainty. If Luna is unsuitable, keep the current Sol unless a capable Terra effort has a clear advantage. Before the first dispatch, read `../../routing-policy.json` once.
4. At the first unresolved choice among multiple capable Sol/Terra routes, run `node ../../scripts/radar-routing-evidence.cjs` once. Keep that compact evidence in the current task and do not run it again. Freeze the selected route; reconsider only if the unit, boundary, availability, or result changes.
...
9. After completion, inspect the actual result or diff and rerun necessary checks in integration order. Return one clear local defect to the same Worker at most once. Capability, scope, or quality failure returns immediately to the current Sol; do not create an execution Sol subagent.
