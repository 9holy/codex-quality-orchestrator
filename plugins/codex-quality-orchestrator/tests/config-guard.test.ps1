...
if (($Args -join ' ') -ceq 'plugin list --json') {
  if ($text.Contains('[plugins."codex-quality-orchestrator@codex-quality-orchestrator"]')) {
    '{"installed":[{"pluginId":"codex-quality-orchestrator@codex-quality-orchestrator","installed":true,"enabled":true,"version":"0.1.4","marketplaceName":"cqo-test","marketplaceSource":{"sourceType":"local","source":"owner/repo"}}]}'
  } else {
    '{"installed":[]}'
  }
  exit 0
...
if ($Args[0] -ceq 'plugin' -and $Args[1] -ceq 'add') {
  ...
  [pscustomobject]@{ pluginId='codex-quality-orchestrator@codex-quality-orchestrator'; version='0.1.4'; installedPath=$env:CQO_TEST_PLUGIN_ROOT } | ConvertTo-Json -Compress
  exit 0
}
