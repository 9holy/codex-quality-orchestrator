...
assert.match(skill, /调用 `spawn_agent` 显式传 `agent_type` 和 `fork_turns`/);
assert.match(skill, /默认 `fork_turns:"none"`/);
assert.match(skill, /`terra_worker` 另传 `reasoning_effort`/);
assert.match(skill, /均不传 `model`/);
assert.match(skill, /`message` 前两行固定为 `\[CQO_WORK_PACKET_V1\]` 和 `route: <model> \/ <effort>`/);
assert.match(skill, /`namedAgents\[selected_agent\]\.model` 和实际 `selected_effort`/);
