...
assert base_version == policy["policyVersion"] == "0.3.14"
...
assert "仅目标/验收明确、低风险、上下文少且可直接验证者为短任务" in rule
assert "高风险不算短任务" in rule
assert "非短任务由当前 `gpt-5.6-sol` 理解、判险、规划和拆解" in rule
assert "仅高置信度确认 `luna_worker`（Luna Max）能独立正确完成" in rule
assert "写入受限、失败可回滚、验收足以发现错误时才下派" in rule
assert "能力/风险门槛通过后优先 Luna Max" in rule
assert "禁止为使用 Luna 强拆耦合任务" in rule
assert "禁止把 Luna 当试错、草稿或先做后修模型" in rule
...
assert "不得向 Luna 下派能力不确定" in rule
assert "Luna 不合格时" in rule
assert "当前 Sol 能可靠完成则接管" in rule
assert "当前 Sol 能力不足且深推理子问题可独立下派时用 Terra Ultra" in rule
assert "主控能力不足则建议下一任务用 Sol XHigh" in rule
assert "Terra XHigh/Max 可显式调用但不进入自动路由" in rule
assert "Worker 结果经 Sol 检查差异并复跑验证后才可接受" in rule
assert "出现能力不足、理解偏差、越界或风险不可验证即停" in rule
assert "按备份和差异处理未接受改动，再按上段门槛接管或升级" in rule
assert "仅明确、局部、低风险的问题可向原代理定向修正一次" in rule
