...
本文件用于维护和回归追踪，不注入模型上下文。`references/RULE16.md` 中的无编号 `Codex Quality Routing` 是唯一语义路由规则，Routing Skill 只实现该规则，Hook 只校验机械字段。

## 目标与路由

| 编号 | 不可丢失的要求 | 落点 |
|---|---|---|
| Q01 | 质量门槛优先；只在能可靠胜任的候选中优化总成本 | Codex Quality Routing、Routing Skill |
| Q02 | 当前 Sol 保持根模型和档位，负责规划、拆解、所有权、整合、验收、裁决和兜底 | Codex Quality Routing |
| Q03 | 清晰、低风险、可直接验证的短任务由当前主代理处理 | Codex Quality Routing |
| Q04 | 冻结、低判断、可机械验证的单元若 Luna Max 能可靠完成且下派有净收益，必须第一优先 | Codex Quality Routing、Policy |
| Q05 | 不把 Luna 用作试错、草稿或先做后修模型，不为调用 Luna 强拆模糊、耦合或高风险决策 | Codex Quality Routing |
| Q06 | 高风险决策留给 Sol；边界和验收冻结后，其中安全的证据或执行单元仍可下派 | Codex Quality Routing |
| Q07 | Luna 不适用后，适合独立下派的常规判断单元优先 Sol Medium；Terra 仅在相对合格 Sol Medium 或当前 Sol 有明确任务优势时使用最低可靠档 | Codex Quality Routing、Routing Skill |
...
| Q10 | IQ 差至少 3 才因质量换高 IQ；同级先保留热模型或原代理，再比较预计总成本 | Codex Quality Routing、Radar |
| Q11 | 每个根任务最多读取一次 Radar；Luna 合格、候选唯一或判断明确时不读取；路由冻结至边界、可用性或结果变化 | Codex Quality Routing、Routing Skill |
...
| T03 | 同质批量先验收一个代表单元，再填满宿主可用容量；无任务累计上限 | Codex Quality Routing、Routing Skill |
| T04 | 共享文件单写者；普通模式 Worker 不下派；爆种模式仅在工作包明确授权且深度小于 d4 时下派 | Codex Quality Routing、代理模板 |
| T05 | Worker 运行时使用一次长阻塞等待，由结果被动唤醒；禁止轮询 | Codex Quality Routing、Routing Skill |
| T06 | Sol 检查实际结果或差异并复跑必要验证，按计划顺序整合 | Codex Quality Routing、Routing Skill |
| T07 | 明确局部缺陷最多续交原 Worker 一次；能力、越界或质量失败立即交回 Sol | Codex Quality Routing |
...
| M04 | 精确容量消息只让同一代理在原上下文继续一次，不重启或重做；第二次交回 Sol 重新路由 | Codex Quality Routing、SubagentStop Hook |
