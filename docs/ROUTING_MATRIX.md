...
在质量、胜任能力和风险边界全部满足后，先保留当前热模型和原代理，再比较预计总算力成本，计入新上下文、重试、返工和复核。只要 Luna Max 能可靠完成且执行量足以摊薄上下文，就优先承担；Luna 不适用后，当前 Sol 能可靠完成且无需独立/并行就直接处理。

Luna Max 的能力门槛是硬优先级：能可靠完成且可独立验收时绝不上调。Codex Radar 只在合格候选间生成稳定优先关系；IQ 差距小于配置区间时先保持热模型/原代理，再选低预计总成本。过期或样本不足的数据不参与路由。
