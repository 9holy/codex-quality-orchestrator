...
Worker 不得越过工作包范围或做无关修改。普通模式不创建子代理；爆种模式仅在工作包明确授权且深度小于 `d4` 时下派。目标或验收不清楚时停止猜测并交还 Sol。所有 Worker 结果都由当前 Sol 验收。
...
普通模式通常只派一个 Worker。仅工作单元互不依赖、写入不冲突且并行收益高于协调成本时并行。大量同质批处理先验收一个代表性单元，再填满宿主可用容量，完成一个就补充一个。Super mode 使用 `d1-d4` 和最多 25 个子线程，`d4` 不再下派；用 `开启爆种模式` / `关闭爆种模式` 或 `enable super mode` / `disable super mode` 控制当前会话。Worker 运行期间使用一次长阻塞等待，由结果提前唤醒；不轮询。共享文件始终只有一个写入者。
...
该恢复只覆盖 `SubagentStop`。主控容量通知不进入当前可续交 Hook，插件不宣称主控自动恢复。
