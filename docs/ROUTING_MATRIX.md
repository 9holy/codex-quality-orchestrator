...
| `terra_worker` | `gpt-5.6-terra / xhigh|max|ultra` | Luna 不适用且适合独立下派的工作；Sol 选择能胜任的最低档位 | 主控职责、架构与最终质量裁决 |
| `sol_reviewer` | `gpt-5.6-sol / xhigh` | 关键高风险变更的一次独立只读复审 | 生产执行、文件写入、并行 reviewer、子代理和最终裁决 |

Sol 判断每个工作单元。Luna Max 能可靠完成且结果可验证就优先下派；不能胜任或判断不确定就不派。在能胜任的候选中，先保留当前热模型和原代理，再比较预计总算力成本。

单个工作单元只要有足够执行量、边界清晰、可以客观验收且交接收益高于协调成本，就可以直接交给一个 Luna Max；只有并发多个单元时才需要团队计划。

Luna Max 只在 Sol 判断能胜任的候选中优先。Codex Radar 只在这些候选间生成稳定优先关系；IQ 差距小于配置区间时先保持热模型/原代理，再选低预计总成本。过期或样本不足的数据不参与路由。

Worker 输出由 Sol 检查差异并复跑验证。明确、局部的问题可交原 Worker 修正一次；能力或质量不合格立即交回 Sol，不按容量错误续交。

## 团队并行

- 单个合格工作单元直接使用一个 Worker；只有至少两个互不冲突的完整工作单元，且并行收益高于协调成本时才组队。
- Sol 在任务开始时一次性确定已知工作单元的执行者；后续按方案派发，仅在新增单元、边界变化、失败或模型不可用时重判。
...
Sol 判断任务语义并生成工作单；由于宿主会在 Hook 前加密 `message`，Hook 只验证明文路由键、调用参数和账本，Sol 必须自己保证工作单内容、权限和备份声明正确。Luna 使用 `selected_effort=max`；Terra 使用 Sol 选定的 `xhigh|max|ultra`；兜底链在调用前声明。
