# 路由矩阵

## 主控

当前 `gpt-5.6-sol` 负责理解、拆解、分派、整合、验收和兜底。插件保持任务创建时已经选择的根档位，不自动把 Medium、High、XHigh、Max 或 Ultra 相互切换，也不创建执行型 Sol 子代理。

| 根档位 | 合理用途 |
|---|---|
| `medium` | 常规规划、拆解、整合和边界明确的直接工作 |
| `high` | 需要更细致检查但不需要复杂推理升级的工作 |
| `xhigh` | 复杂规划、跨模块整合和严格验收 |
| `max` | 疑难、高风险、架构和关键裁决 |
| `ultra` | 系统性、超复杂的长任务，不作为日常默认 |

## Worker

| 代理 | 固定模型 | 档位 | 使用条件 |
|---|---|---|---|
| `luna_worker` | `gpt-5.6-luna` | `max` | 目标、范围和验收明确，Luna 能可靠独立完成；满足时第一优先 |
| `terra_worker` | `gpt-5.6-terra` | `xhigh/max/ultra` | Luna 不适用，需要独立深推理；选择最低可靠档 |
| `sol_reviewer` | `gpt-5.6-sol` | `xhigh` | 关键高风险变更的一次只读复审 |

Worker 不得越过工作包范围、做无关修改或创建子代理。目标或验收不清楚时停止猜测并交还 Sol。所有 Worker 结果都由当前 Sol 验收。

## 选择顺序

1. 短任务由当前主代理完成。
2. 非短任务先判断 Luna Max 是否可靠胜任。
3. Luna 胜任就直接使用，不读取 Radar，也不上调 Terra 或 Sol 子代理。
4. Luna 不适用时，当前 Sol 可直接处理；适合独立下派的深推理单元使用 Terra 最低可靠档。
5. 关键高风险变更需要独立性时增加一次只读 Reviewer。
6. Sol 检查实际差异、复跑验证并作最终裁决。

## 并行

通常只派一个 Worker。仅工作单元互不依赖、写入不冲突且并行收益高于协调成本时并行。大量同质批处理先验收一个代表性单元，再使用宿主当前可用并发，并可按需继续后续批次。任务级 Worker 总数、批次数和累计尝试次数不设硬上限；共享文件始终只有一个写入者。

## 调用契约

```text
luna_worker: agent_type + task_name(luna_max__unit) + fork_turns
terra_worker: agent_type + reasoning_effort(xhigh|max|ultra) + task_name(terra_<effort>__unit) + fork_turns
sol_reviewer: agent_type + task_name(sol_reviewer_xhigh__unit) + fork_turns
```

具名代理不传 `model`。默认 `fork_turns:"none"`，确需少量历史时使用正整数字符串。Hook 只校验宿主可见的确定性调用字段，不判断任务语义，也不阻止其他插件使用自己的代理。

## 容量恢复

精确容量错误首次出现时在原代理上下文中“继续”一次；第二次失败交回当前 Sol。不重新启动整项任务，不重做已经完成的工作，能力或质量失败不得按容量错误处理。
