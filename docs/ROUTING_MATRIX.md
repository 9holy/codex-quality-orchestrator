...
普通模式通常只派一个 Worker。仅工作单元互不依赖、写入不冲突且并行收益高于协调成本时并行。大量同质批处理先验收一个代表性单元，再填满宿主可用容量，完成一个就补充一个。爆种模式按会话开启，使用 `d1-d4` 和最多 25 个子线程，`d4` 不再下派。Worker 运行期间使用一次长阻塞等待，由结果提前唤醒；不轮询。共享文件始终只有一个写入者。
