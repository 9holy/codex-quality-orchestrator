...
| `terra_worker` | `gpt-5.6-terra / xhigh` | 常规实现判断、根因诊断、跨上下文推断和复杂调试 | 架构与最终质量裁决 |
| `terra_worker` | `gpt-5.6-terra / max` | 高难度实现和复杂多文件调试 | 架构与最终质量裁决 |
...
Luna Max 的能力门槛是硬优先级：能可靠完成且可独立验收时绝不上调。Codex Radar 的 IQ、平均成本、耗时和样本数只比较 Luna 不适用后的语义合格候选；IQ 差距小于配置的稳定区间时视为近似持平并优先低成本，过期或样本不足的数据不参与路由。

## 团队并行

- 只有至少两个互不冲突的完整工作单元，且并行收益高于协调成本时组队。
- 通常每波 1 个 Worker；只有并行收益大于额外算力和整合成本时才使用 2–3 个，最多 3 个。根任务累计最多 8 次 Worker 调用，不为使用代理拆分短任务。
