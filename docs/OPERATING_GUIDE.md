...
同一工作单元的执行者和最低能力层级保持稳定，只在实质边界变化、上下文越界、有限修正失败或链路不可用时向上升级。无法安全拆分、Luna/Terra 不能可靠胜任、有限修正仍失败或全局集成必须由主控处理时，当前 Sol 直接接管，不创建执行型 Sol 子代理。只有关键高风险变更需要独立性时才使用一次只读 Sol XHigh reviewer。
