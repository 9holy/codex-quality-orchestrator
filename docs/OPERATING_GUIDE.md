...
Luna Max 在 Sol 判断能胜任的候选中第一优先，雷达数据不能替代 Sol 的判断。SessionStart 只提供一次稳定优先关系；Sol 用它确定已知工作单元的执行者，后续按方案派发，仅在新增单元、边界变化、失败或模型不可用时重判。IQ 差距小于稳定区间时先保留热模型和原代理，再比较预计总成本；无新鲜证据时按静态能力矩阵执行。

短任务必须同时满足目标与验收无歧义、低风险、无需方案选择或诊断、上下文很少且可直接验证；改动大小、文件数量或验证步骤数量只能作为辅助信号。架构、安全、公共接口、生产数据、不可逆操作等高风险事项无论大小都不是短任务。

对每个非短任务，Sol 使用日常路由 Skill 判断每个工作单元由谁执行。一个有足够执行量、交接有净收益且 Luna Max 能可靠完成并验证的单元就直接下派，不要求存在第二个并行单元；不适用时由当前 Sol 处理，或把适合独立下派的单元交给能胜任的 Terra 最低档位。最终审核、架构、高风险、生产数据、跨代理最终集成与最终裁决保留给 Sol。
...
通常每波只使用 1 个 Worker；单个合格工作单元不构造团队计划。只有至少两个工作单元可以安全并行，且收益高于额外算力和协调成本时才使用 2–3 个，最多 3 个；Luna 可以在三个互不冲突的清晰执行单元上占满并发。每次调用的 `task_name` 使用 `<模型档位>__<单元>__w<波次>__s<槽位>of<波次大小>__a<尝试>` 明文路由键，`message` 携带给 Worker 阅读的冻结 `CQO_WORK_PACKET_V1` JSON 工作单，由 Sol 负责共享文件单写者。Worker 不得创建或下派子代理。
...
`codex-model-routing-team` 由 Lead 冻结 RoutePlan、候选链、Provider/Surface/模型能力，并用 preflight 和 team ledger 验证线程实体化、唯一任务、并发、重试和关闭状态；它默认 App Thread，并以至少两个独立交付物作为自动组队门槛。当前环境稳定暴露原生具名 Worker，因此本插件采用 Sol 语义路由 + `CQO_WORK_PACKET_V1` + 原生 Hook 会话账本：保留冻结工作单、任务意图/写权限、预声明 fallback、唯一任务、波次、并发上限、尝试预算、生命周期和一次容量续交，同时允许单个有净收益的 Luna 工作单；不复制本环境不需要的 App Thread、Provider/Surface、Fast、第三方模型分支或累计八次上限。
...
- 规则匹配且代理配置完整时输出 `[CQO_ACTIVE]` 和一条精简 `[CQO_ROUTE]` 提醒；不重复注入 Rule 16，也不报告可能被任务选择器覆盖的根默认值。SessionStart 会在上下文压缩后重新运行，因此无需每轮重复注入。
