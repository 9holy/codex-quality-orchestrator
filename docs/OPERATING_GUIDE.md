...
Luna Max 能可靠完成且可独立验收时绝不上调，雷达数据不得覆盖此规则。Luna 不适用时，SessionStart 可提供严格白名单化的 Codex Radar 数值快照；Sol 先筛出语义合格候选，再参考 IQ、平均成本、耗时、样本量和新鲜度选择预计总成本最低者。IQ 差距小于稳定区间时优先低成本；无新鲜证据时完全按静态能力矩阵执行。
...
快照只保留允许模型的数值字段，默认缓存 6 小时、最长使用 24 小时；请求有超时和大小限制，写入采用原子替换。外部任务标题、推荐文案和其他自由文本不会进入 Sol 上下文。
